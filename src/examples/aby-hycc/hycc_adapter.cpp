...
bool test_cbmc_circuit(Options const &options, seclvl seclvl, uint32_t nthreads, e_mt_gen_alg mt_alg)
{
	...
	std::cout << "LOG: " << (options.role == SERVER ? "Server load time: " : "Client load time: ") << load_time.count() << std::endl;
	...
	std::cout << "LOG: " << (options.role == SERVER ? "Server exec time: " : "Client exec time: ") << exec_time.count() << std::endl;
